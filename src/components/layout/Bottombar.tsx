    ...
    <nav className="fixed bottom-0 inset-x-0 mx-auto max-w-md z-[100] bg-white pb-safe border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
      <ul className="flex justify-around items-center h-20 px-4">
        {navItems.map((item) => {
          ...
          const isActive = !item.isExternal && (item.href === '/' ? pathname === '/' : pathname.startsWith(item.href) || (item.label === 'Tienda' && pathname.startsWith('/tienda')));
          
          const commonClasses = `flex flex-col items-center gap-1.5 min-w-[64px] transition-all duration-300 ${
