...
_Pedido generado desde Favorit AI_`;
  ...
  return (
    <div className="min-h-screen bg-[#F5F0EB] pb-72">
      ...
      {/* Sticky Bottom Bar */}
      <div className="fixed bottom-24 left-0 right-0 z-40 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] rounded-t-[32px] px-6 pt-5 pb-8 max-w-2xl mx-auto">
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center px-1">
            <span className="text-sm font-medium text-gray-400">Subtotal</span>
            <span className="text-sm font-bold text-gray-400">${total.toLocaleString('es-AR')}</span>
          </div>
          ...
          <div className="flex justify-between items-baseline px-1 mt-1 text-[#2D6A4F]">
             <span className="text-sm font-medium text-gray-400">Puntos a sumar</span>
             <span className="text-xl font-black">+{Math.floor(total / 1000)}</span>
          </div>

          <button
            onClick={handleCheckout}
            disabled={loading}
            className="w-full h-14 bg-brand-borravino text-white rounded-2xl font-bold text-base flex items-center justify-center gap-3 shadow-[0_4px_15px_rgba(107,33,57,0.4)] active:scale-[0.98] transition-all hover:brightness-110 uppercase tracking-widest"
          >
